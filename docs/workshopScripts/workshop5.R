...
# use map and custom function using same initial conditions
pop4 = tibble(rds = c(1.3, 1.6, 1.9, 2.2, 2.5, 2.8)) |> 
  mutate(pop = map(rds, \(rd) dlogisticD(K = 900, rd = rd, N_0 = 100, years = 50)))
...
pop4
#Plot the results 
ggplot(pop4, aes(time, Nt, color = as.factor(rds))) +
  geom_line(linewidth = 1)+
  labs(y = expression(italic(N[t])),
       x = 'Time',
       color = expression(italic(r[d])))+
  facet_wrap(~rds, nrow = 2)+
  theme_bw()+
  theme(legend.position = 'bottom',
        legend.text = element_text(size = 8))
